...
import { Contact, Membership, STAGES, PRIORITIES, TIERS, CONTACT_TYPES, ContactFilters } from '@/lib/types';
import { formatCurrency, forecast, isOverdue, tierColor, stageColor, downloadCSV } from '@/lib/utils';
...
type SortCol = 'display_name' | 'tier' | 'steward' | 'stage' | 'ask_amount' | 'forecast_date' | 'next_action_date';
...
export function ContactsListView({ initialContacts, memberships, orgSlug, orgId }: Props) {
  ...
  const hasFilters = !!(filters.search || filters.steward || filters.stage.length || filters.tier.length);
          ...
          <table className="w-full">
            <thead>
              <tr>
                ...
                <SortHeader col="forecast_date" label="Forecast Date" />
            ...
            <tbody>
              ...
              {filtered.map(c => (
                ...
                >
                  ...
                  <td className="table-cell text-xs tabular-nums text-gray-600">{c.forecast_date || '—'}</td>
