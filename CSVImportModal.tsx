...
import { bulkCreateContacts } from '@/lib/queries';
...
export function CSVImportModal({ orgId, memberships, onComplete, onClose }: Props) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const result = await bulkCreateContacts(supabase, orgId, contacts);
    ...
    <Modal title="Import Contacts from CSV" onClose={onClose}>
      ...
      {step === 'done' && (
        <div className="text-center py-6">
          <div className="text-4xl mb-3">{importResult.errors === 0 ? '✓' : '⚠'}</div>
          <h3 className="font-bold text-lg mb-2">
            {importResult.errors === 0 ? 'Import Complete' : 'Import Finished with Errors'}
          </h3>
          <p className="text-sm text-gray-600 mb-1">
            <strong className="text-emerald-700">{importResult.success}</strong> contacts imported successfully.
          </p>
          {importResult.errors > 0 && (
            <p className="text-sm text-amber-700">{importResult.errors} rows failed.</p>
          )}
